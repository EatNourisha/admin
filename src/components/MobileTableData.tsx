...
type FieldConfig = {
  title: string;
  // Render function that takes the row data and returns a React element
  render: (data: any) => React.ReactNode;
  align?: "left" | "right";
};

// Type for the field pairs that will be displayed in rows
type FieldPair = {
  left: FieldConfig;
  right: FieldConfig;
};
interface MobileTableDataProps extends BoxProps {
  ...
  // Array of field pairs to be displayed
  ...
  if (!data.length) {
    return (
      <VStack maxW="200px" mx="auto" my="180px" hideFrom={"md"}>
        {emptyState?.image && <Image src={emptyState.image} alt="empty list" boxSize="150px" />}
        <Text textAlign="center" fontSize="14px">
          {emptyState?.message ?? "Sorry, it looks like you have nothing here yet"}
        </Text>
      </VStack>
    );
  }
